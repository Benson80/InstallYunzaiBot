...
if [ "$choice_manage" == "1" ]; then
   ...
   # 等待用户确认
    ...
    # 更新 bot.yaml 文件
    sign_api_addr="http://$ip2:$port/sign?key=114514"
    ver="$txlib_version"
    # 将新的地址和版本写入配置文件
    sed -i "s|^sign_api_addr:.*$|sign_api_addr: $sign_api_addr|" /root/Miao-Yunzai/config/config/bot.yaml
    sed -i "s|^ver:.*$|ver: $ver|" /root/Miao-Yunzai/config/config/bot.yaml
fi